...
| `kirocrew doctor --ledger-sweep --purge [--older-than-days N] [--include-orphans] [--purge-unreadable]` | Delete the ledgers the dry run listed. Irreversible, explicit, and never automatic — no gateway hook, no history-delete path and no MCP tool reaches it. Default idle window 30 days; an unreadable record is listed but kept unless `--purge-unreadable` is given, and a store with no `slot_key` breadcrumb is never purged. Rules and rationale: [session-work-ledger](session-work-ledger.md#cleanup). |

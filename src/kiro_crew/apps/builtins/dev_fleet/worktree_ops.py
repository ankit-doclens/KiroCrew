...
def _discard_dir(target: Path) -> None:
    """Remove *target* and its contents, tolerating a path that is already gone."""
...
async def _release_channel_advance(lane: str) -> dict:
    ...
    * The **live-target binding**, for the same reason: it names a path that
    ...
    async with lock:
        ...
        # An artifact built from a revision this tree is not on describes code that
        # is not here. Drop exactly the ones whose inputs changed; provisioning
        # rebuilds those and keeps the rest.
        rebuilt: list[str] = []
        ...
        for rel in stale:
            ...
            try:
                ...
                rebuilt.append(rel)
            except Exception:  # pragma: no cover - defensive; advance itself succeeded
                runtime.logger.exception("could not invalidate %s after advance", stale_dir)
    return {
        "ok": True,
        "lane": lane,
        "name": name,

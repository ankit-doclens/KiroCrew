...
| `diff_signals.py [base] [--check-body FILE]` | 1 / 2 / 3 | changed files + flagged signals (deps, lockfiles, migrations, CI, deletions, config). With `--check-body` it also runs the two body checks: **accounting** — every changed area (the `pr-scope.yml` unit) must be named in the body — and the **soft** `What changed` prose length (default 400 words; fences, tables and images excluded), which only WARNs. The body file must resolve under the git dir, or under the worktree outside any hidden directory | **0 · 20 unaccounted area (`--check-body` only) · 2 env / body file outside those roots** |
...
`diff_signals.py --check-body <body-file>` reads the finished body and applies
two rules of different weight. Keep them apart in your head: one is about
**accounting**, the other about **reading**.

| check | what it measures | on breach | why that strength |
|---|---|---|---|
| Accounting | every changed area (the unit `pr-scope.yml` counts: a module directory under `src/kiro_crew/` or `website/src/`, the top-level component elsewhere) is named in the body — by the area, a changed path or its `dir/file` tail, or a unique non-generic file name | **exit 20** — stop, fix the body or the diff | a change nobody is told about is exactly how an unrelated edit rides along |
| Length | words of prose in `What changed`, with fenced blocks, table rows and image lines excluded | **WARN**, exit 0 | a rename or a shared-helper migration needs the words; a hard cap would cut true facts to hit a number |

The accounting is the leak detector, not the prose. A long walkthrough hides a
stray file better than a short body does, because the reviewer trusts the
walkthrough and skips the diff. So: short prose, complete ledger. Paths, tables
and pictures never count against the soft limit — put the accounting there, and
keep the paragraphs for the point.

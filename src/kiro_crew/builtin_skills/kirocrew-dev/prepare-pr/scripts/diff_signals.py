...
With ``--check-body FILE`` it also reads a PR body and runs two checks on it:
...
* **Accounting (hard).** Every changed *area* -- the unit ``pr-scope.yml`` counts,
  ...
  The body file itself must resolve (symlinks followed) to a path under the
  worktree or its git directory (``git rev-parse --git-dir``, which is where the
  skill writes it), so the script cannot be pointed at a file outside them; both
  roots come from git, never from an environment variable a caller could bend.
  Inside those roots a hidden directory component (``.aws``, ``.ssh``, ...) is
  refused too, except under the git directory itself: a dotfiles repository
  checked out at ``$HOME`` would otherwise make every credential file "inside the
  worktree". Anything else is exit 2.
* **Length (soft).** The prose of ``## What changed`` -- fenced blocks, table rows
  and image lines excluded -- is counted against ``SOFT_WORDS`` (400). Over the
  limit prints a WARN and still exits 0: a wide cross-cutting change legitimately
  needs the words, so the limit is a nudge, never a gate.

Usage:  python3 diff_signals.py [base-branch] [--check-body FILE]
...
def _under(real, root):
    """True when the already-resolved ``real`` lies under directory ``root``."""
    if not root:
        return False
    r = os.path.realpath(root)
    try:
        return os.path.commonpath([real, r]) == r
    except ValueError:
        return False  # different drives on Windows


def body_path_allowed(path, toplevel, gitdir):
    """True when ``path`` resolves (symlinks followed) to a body file this script may read.

    Allowed: anything under the git directory, or under the worktree with no
    hidden (dot-prefixed) directory component below the worktree root. The body
    file is written to one of those two places; nothing else is a legitimate
    target, and the diff being checked can carry attacker-chosen names, so a
    caller must not be able to aim this read anywhere else.

    Roots are git-derived on purpose: an environment-derived root such as
    ``TMPDIR`` is one export away from pointing at a sensitive directory. The
    hidden-component rule covers the other way in: a dotfiles repository checked
    out at ``$HOME`` makes ``~/.aws/credentials`` "inside the worktree", and no
    PR body lives in a hidden directory other than ``.git``.
    """
    real = os.path.realpath(os.path.expanduser(path))
    if _under(real, gitdir):
        return True
    if not _under(real, toplevel):
        return False
    rel = os.path.relpath(real, os.path.realpath(toplevel))
...
def parse_args(argv):
    ...
    ap.add_argument("--check-body", default=None)
...
def main(argv):
    ...
    body = None
    if args.check_body is not None:
        toplevel = run(["git", "rev-parse", "--show-toplevel"])[1].strip()
        gitdir = run(["git", "rev-parse", "--absolute-git-dir"])[1].strip()
        if not (toplevel and gitdir) or not body_path_allowed(args.check_body, toplevel, gitdir):
            err(
                "ERROR: --check-body must point inside the worktree (outside any hidden "
                "directory) or inside its git directory (got {})".format(args.check_body)
            )
            return 2
        try:
            with open(
                os.path.realpath(os.path.expanduser(args.check_body)),
                encoding="utf-8",
                errors="replace",
            ) as fh:
                body = fh.read()
        except OSError as exc:
            err("ERROR: cannot read body file {}: {}".format(args.check_body, exc))
            return 2

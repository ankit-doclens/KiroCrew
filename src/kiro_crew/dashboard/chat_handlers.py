...
async def api_chat_slot_create(request: web.Request) -> web.Response:
    ...
    if instance_id and adopt_remote_slot:
        ...
        # The peer's mode wins over the request's. It is the user's privacy
        # boundary and the peer session already has one, so defaulting it here
        # would let a session opened as `incognito` over there start writing
        # memory the moment it was opened on this machine. Safe past the
        # validation above because `peer_row_metadata` emits only a value from
        # that same allowlist.
        memory_mode = peer_meta.get("memory_mode", memory_mode)
        ...
        # what the adopted session claims to be. Unconditional rather than
        # `if not agent`: the adopt path has no agent picker, so a caller-supplied
        # agent here could only disagree with the session being adopted — it would
        # open the peer's conversation under a different agent than the one that
        # has been answering in it. Stored VERBATIM: the surrounding
        # resolve/normalize steps are skipped for every peer-bound create precisely
        # because they answer from THIS machine's roster.
        agent = peer_meta.get("agent", "") or agent

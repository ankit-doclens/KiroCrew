...
#: The two notices a backfill can leave in the transcript. Text, not codes:
...
_BACKFILL_FAILED_NOTICE = (
    "This session's earlier history could not be copied from the crew. "
    "The conversation continues there, and the crew still answers with it."
)
_BACKFILL_TRUNCATED_NOTICE = (
    "Older messages in this session stay on the crew and are not copied here."
)
_BACKFILL_IN_FLIGHT_NOTICE = (
    "The crew is still answering in this session. That reply is not copied here; "
    "it finishes on the crew."
)
...
async def fetch_adopted_backfill(
    ...
    try:
        ...
        return AdoptBackfill([], _BACKFILL_FAILED_NOTICE)
    except Exception:
    ...
    # Both conditions can hold at once — a long session the crew is still
    ...
    notices = [
        ...
        for text, on in (
            ...
            (_BACKFILL_IN_FLIGHT_NOTICE, in_flight),
        )
        if on
    ]

...
from kiro_crew.dashboard.peer_session_index import peer_identity
...
def resolved_row_identity(slot: Any) -> str:
    ...
    the same identity the peer row carried before anything was bound to it.
    ...
    Shares :func:`~kiro_crew.dashboard.peer_session_index.peer_identity` with the
    hub-side resolver rather than formatting the pair here, so the two cannot drift
    into disagreeing about what a row is called.
    """
    ...
    if getattr(slot, "is_remote", False) and instance_id and remote_slot:
        return peer_identity(instance_id, remote_slot)

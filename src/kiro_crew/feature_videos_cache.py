...
def resolve_served_path(release: str, name: str) -> "tuple[Path, str] | None":
    ...
    which then proves on the descriptor itself that what was opened sits at this
    canonical path — so a file OR a folder swapped for a link between this check
    and the open is refused, and a swap after the open changes nothing the
    response reads. Residual, stated: a local process with the user's own
    permissions can still replace the FILE's bytes in place; the receipt
    certifies what was installed, not what is read back, and the user doc says
    so.
    """
...
def _open_served(path: Path) -> "tuple[int, int] | None":
    ...
    The open IS the final check, and everything after it is asked of the
    descriptor, never of the path:

    * ``open_file_no_reparse`` refuses a link at the LEAF in the same operation
      that opens it (``O_NOFOLLOW`` on POSIX, the reparse-point flag on Windows);
    * the descriptor's real path (:func:`kiro_crew.pinned_fs.fd_real_path`) must
      be *path* itself — the canonical ``<root>/<release>/<name>`` the resolver
      built. That is what settles the DIRECTORIES: the leaf rule cannot see a
      release folder or the root swapped for a link after the resolver's lstat,
      because the open follows an ancestor link and finds an ordinary file at the
      end. The kernel's answer for the inode already open cannot be re-pointed,
      so a mismatch means the open went somewhere else and is refused. A
      platform that cannot read it refuses too;
    * ``st_nlink`` must be 1. A hard link is invisible to every path rule — it
      IS the inode, under a second name — so a file linked into the cache from
      outside it would pass both checks above. Nothing this cache installs has a
      second name;
    * the type and size come off the same ``fstat``. Non-blocking, so a planted
      FIFO is rejected by that ``fstat`` instead of blocking the open.

    A swap after this returns changes nothing the response reads.
    """
    try:
    ...
    if (
        ...
        or st.st_nlink != 1
        or real is None
        or os.path.normcase(real) != os.path.normcase(str(path))
    ):
        os.close(fd)
        return None
...
async def api_feature_video_file(request: web.Request) -> web.StreamResponse:
    ...
    resolved = await asyncio.to_thread(
        resolve_served_path,
        request.match_info.get("release", ""),

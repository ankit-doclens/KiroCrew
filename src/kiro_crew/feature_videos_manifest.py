...
def ensure_cache_dir(release: str) -> Path:
    ...
    Nothing here is written BY NAME after a check. The folder is created relative
    to the pinned cache root and opened with no-follow semantics in the same step
    (:func:`kiro_crew.pinned_fs.create_and_open_dir_pinned`), the open descriptor
    has to prove it really is ``<canonical root>/<release>``
    (:func:`kiro_crew.pinned_fs.fd_real_path`), and only then is the folder
    tightened — through that descriptor (``fchmod``) on POSIX, and on Windows by
    name under the held handle, which blocks a rename of the folder and everything
    above it for as long as it is open. A ``chmod`` by name after a by-name check
    would follow a link planted in between and land ``0700`` on whatever it points
    at; that write has no undo, so the check is settled on the thing already held.
    ...
    # no-follow open and the descriptor witness below are what settle it.
    _refuse_link(path, "feature-video release folder")
    fd = _create_release_dir_held(path)
    try:
        try:
...
def _create_release_dir_held(path: Path) -> int:
    """Create (or meet) the release folder and return it OPEN, never having followed a link.

    POSIX: :func:`kiro_crew.pinned_fs.create_and_open_dir_pinned` pins the resolved
    cache root one ``openat`` at a time, creates the leaf relative to it and opens
    the leaf with ``O_NOFOLLOW`` — a link or a file at the name is refused as
    :class:`CacheDirRefused`. Windows has no ``dir_fd``: the folder is created by
    name (``mkdir`` writes through nothing), refused if the name is a link,
    junction or file, and pinned with the handle that blocks its rename
    (:func:`kiro_crew.platform_compat.pin_directory`).
    """
    if pinned_fs.supports_pinned_walk():
        return pinned_fs.create_and_open_dir_pinned(
            path, what="feature-video release folder", refusal=CacheDirRefused
        )

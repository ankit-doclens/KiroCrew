...
def check_memory_available(
    ...
    production, so routing it through the gate added no protection while a
    gate refusal under load silently disabled spawn back-pressure exactly
    when it mattered (#9894). ``platform_compat._linux_available_mib`` reads
    the same file the same way. The ``path`` keyword is keyword-only and
    exists for tests only; production callers always take the constant.

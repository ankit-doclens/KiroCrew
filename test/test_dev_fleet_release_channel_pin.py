...
# naming
# --------------------------------------------------------------------------
@pytest.mark.parametrize("lane", ["stable"])
...
def test_tag_lane_decides_by_shape_and_never_disagrees_with_the_shared_rule(tag):
    """The regex IS the classification here, and it answers what the repo answers.

    ``tag_lane`` does not call ``release_channel.channel``: a bare ``vX.Y.Z`` has no
    suffix left to read, so the call could only agree. What still has to hold is
    that the two never DIVERGE — a release-shaped tag this module admits as
    ``stable`` must be one the shared rule also calls stable, or the lane would pin
    to a build the rest of the product does not treat as a release.
    """
    from kiro_crew import release_channel

    assert rcp.tag_lane(tag) == "stable"
...
def test_tag_lane_rejects_anything_that_is_not_a_stable_release_tag(tag):
    """A prerelease tag is not a release tag here, because no lane can answer as one.

    ``stable`` is the only entry in :data:`LANES`, so a prerelease could only ever
    be classified and then discarded. Rejecting it at the shape check keeps one
    path instead of two that agree.
    """
...
@pytest.mark.asyncio
async def test_resolve_refuses_a_lane_this_app_does_not_materialize(monkeypatch):
    """``insider`` classifies as a channel and still is not a LANE.

    ``tag_lane`` keeps calling its tags insider, because that is what the update
    stack's classifier says and this module does not get a second opinion. What
    changed is which lanes Dev Fleet offers: only those whose tag ORDER is a fact.
    Ranking ``-insider.N`` against ``-rc.N`` needs a precedence nothing in this
    repo states, so the lane is refused here rather than resolved by a rule this
    feature invented.
    """
    monkeypatch.setattr(runtime, "_run_cmd", _fake_git())
    got = await rcp.resolve("insider")
    assert got["ok"] is False
    assert "insider" in got["error"]
...
def test_lanes_are_real_release_channels():
    """A lane must name a specific PUBLISHED build whose order among its tags is a fact.
    ...
    ``insider`` fails the second half rather than the first: its tags resolve, but
    ranking ``-insider.N`` against ``-rc.N`` needs a precedence nothing in this
    repo states. Both are excluded, for reasons that are not the same reason —
    which is why ``LANES`` is spelled out rather than filtered.
    """
    ...
    assert "insider" not in rcp.LANES
    assert rcp.LANES == ("stable",)
...
@pytest.mark.asyncio
async def test_resolve_does_not_re_decide_the_lane_it_already_picked(monkeypatch):
    ...
    verified its own answer. It could not: selection already filtered on
    ``tag_lane(tag)``, and ``version`` is that same ``tag[1:]`` — so the comparison
    was a tautology that only ever agreed, and the test which "proved" it fired had
    to stub BOTH sides to produce a disagreement. A guard asserted against a stub
    of itself is not a guard.

    What replaces it: the lane is decided exactly once, by the tag's shape, and the
    result carries no field re-stating it.
    """
    ...
    assert got["version"] == "0.5.0"
    assert rcp.tag_lane("v0.5.0") == "stable"

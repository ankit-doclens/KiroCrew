...
class TestTheTwoSidesFoldCaseIdentically:
    ...
    wrong gap. Python's `casefold()` is the more thorough fold and that is exactly
    why it is wrong here: it maps `ß` to `ss` where JavaScript's `toLowerCase`
    leaves it, so an equal-order pair would order one way for the tool and the
    other way for the person.
    """

    def test_sharp_s_is_not_folded_to_ss(self) -> None:
        rows = [
            {"id": "aaaaaaaaaaaa", "name": "stra\u00dfe", "parent_id": "", "order": 0},
            {"id": "bbbbbbbbbbbb", "name": "strasse", "parent_id": "", "order": 0},
        ]
        names = [str(f["name"]) for f in mcp_dashboard._chat_folder_siblings(rows, "")]
        # Lowercased, 'ss' sorts before 'ß' by code point. Under casefold the two
        # names collapse to one string and the order falls to input position.
        assert names == ["strasse", "stra\u00dfe"]

    def test_the_key_matches_a_plain_lowercase(self) -> None:
        """Pins the fold itself, so a future edit back to casefold is caught."""
        rows = [
            {"id": "aaaaaaaaaaaa", "name": "\u00dfeta", "parent_id": "", "order": 0},
            {"id": "bbbbbbbbbbbb", "name": "sseta", "parent_id": "", "order": 0},
        ]
        ordered = [str(f["name"]) for f in mcp_dashboard._chat_folder_siblings(rows, "")]
        assert ordered == sorted((str(r["name"]) for r in rows), key=str.lower)
...
class TestFolderPosition:
    ...
    def test_a_failed_order_write_says_the_move_itself_landed(self) -> None:
        """The two halves have different outcomes, so they get different words.

        Re-moving a folder that already arrived is the mistake this prevents.
        """
        calls: list[dict] = []
        ...
        with (
            ...
            out = _call_tool_inner("chat_folder_move", {"folder": "Delta", "after": "Alpha"})
        assert "Moved folder" in out and "stopped partway" in out
        assert not out.startswith("Error:")

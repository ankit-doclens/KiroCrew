...
class TestInheritedMetadata:
    ...
    async def test_an_unrecognised_peer_mode_is_dropped_not_coerced(self, tmp_path, no_mint):
        ...
        assert state._slots[body["key"]].memory_mode == "persistent"
...
class TestBackfill:
    ...
    async def test_an_oversized_full_read_falls_back_to_a_bounded_tail(
        ...
        assert "stay on the crew" in rows[0]["content"]
    ...
    async def test_the_row_cap_keeps_the_tail_and_says_so(self, tmp_path, no_mint, monkeypatch):
        ...
        rows = state._slots[body["key"]].messages
        assert [m["content"] for m in rows] == [ra._BACKFILL_TRUNCATED_NOTICE, "m3", "m4", "m5"]

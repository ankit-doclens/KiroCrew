...
describe('bySidebarOrder', () => {  /**
  ...
  it('folds case the way the Python sort key does, so equal orders agree', () => {
    // `casefold()` maps ß to ss and `toLowerCase()` does not, so the two sides
    // would order an equal-order pair differently. Both now lower.
    const rows = [f('sharp', 'straße', 0), f('ss', 'strasse', 0)]
    // 'straße' vs 'strasse' lowercased: 'ss' < 'ß' by code point, so strasse first.
